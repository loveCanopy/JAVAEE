...
public class InsertCustomerFrame extends JFrame {
	...
	protected void do_insertButton_AddCustomer() {
		...
		if ( CustomerNameJTextField.getText() != null && !CustomerNameJTextField.getText().equals("") 
			...
			if ( i == 1 ) {
	            JOptionPane.showMessageDialog(getContentPane(), "成功新增顧客資料！", "", JOptionPane.WARNING_MESSAGE);
	            return;
			} else {
	            JOptionPane.showMessageDialog(getContentPane(), "新增顧客資料失敗！", "", JOptionPane.WARNING_MESSAGE);
	            return;
			}
		} else {
